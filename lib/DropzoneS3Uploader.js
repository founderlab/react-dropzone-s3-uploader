...
var DropzoneS3Uploader = (function (_React$Component) {
  ...
  function DropzoneS3Uploader() {
    ...
    this.onProgress = function (progress) {
      if (_this.props.onProgress) _this.props.onProgress(progress);
      _this.setState({ progress: progress });
    };

    this.onError = function (err) {
      if (_this.props.onError) _this.props.onError(progress);
      _this.setState({ error: err });
    };

    this.onFinish = function (info) {
      if (_this.props.onFinish) _this.props.onFinish(info);
